...
describe("getAntiConsumeristMultiplier", () => {
  it("returns 1 without upgrade level", () => {
    ...
    expect(getAntiConsumeristMultiplier(shop, Date.now())).toBe(1);
  });
  ...
  it("returns 1 when wall clock ms is not finite", () => {
    ...
    expect(getAntiConsumeristMultiplier(shop, Number.NaN)).toBe(1);
  });
  ...
  it("ramps linearly from x1 and caps at tier value", () => {
    ...
    const maxMult = tier.value;
    expect(getAntiConsumeristMultiplier(shop, lastMs)).toBe(1);
    expect(getAntiConsumeristMultiplier(shop, lastMs + (durationSec / 2) * 1000)).toBeCloseTo(1 + (maxMult - 1) / 2, 6);
    expect(getAntiConsumeristMultiplier(shop, lastMs + durationSec * 1000)).toBe(maxMult);
    expect(getAntiConsumeristMultiplier(shop, lastMs + durationSec * 10 * 1000)).toBe(maxMult);
  });
});
